...
import { MatchImportData, SyncPhase, TeamRankingImportData } from "./data_access/import_datastructures";
...
export async function sync(): Promise<void> {
  ...
  const nMatchesSynced = await syncMatches(matchesToSync);
  // await syncTeamRanking();
  // await 
 ...
 */
async function syncMatches(matchesToSync: Match[]): Promise<number> {
  let nSynced = 0;
  ...
  for (let match of matchesToSync) {
    ...
    let isMatchSynced: boolean = await appdata.setMatch(match);
...
async function syncTeamRanking(matchesToSync: Match[]): Promise<boolean> {
 ...
 */
async function getRelevantMatchesToSync(): Promise<Match[]> {
 ...
 */
async function getRelevantSyncPhases(): Promise<SyncPhase[]> {
 ...
 */
async function getMatchesFromSyncPhase(syncPhase: SyncPhase): Promise<Match[]> {
...
/**
 * Yields fresh data from the reference data (openligadb) for all the
 * given matches
 * 
 * @private
 ...
 */
async function getReferenceMatchData(matches: Match[]): Promise<MatchImportData[]> {
...
/**
 * Enrichtes the given Match with live data from the MatchImportData
 * 
